...
def handle_local(session: Session, args: Namespace) -> None:
...
def handle_api(args: Namespace) -> None:
    from api import add_book, rent_book, return_book, add_person, \
        update_person, remove_person, load_data, list_books, list_people
    try:
        if args.model == "books":
            if args.add:
                if not all([args.title, args.year, args.author]):
                ...
                for b in books:
                    print(
                        f"{b['author']} {b['title']} ({b['year']}); avalible: {b['avalible']}")
