...
class Book(Base):
    ...
    borrower_id: Mapped[Optional[int]] = \
        mapped_column(ForeignKey('People.id'), nullable=True)
    borrower: Mapped[List[Person]] = \
        relationship('Person', back_populates='borrowed_books')
    ...
    @validates("year")
    def validate_year(self, key, year: int) -> int:
...
class Person(Base):
    ...
    borrowed_books: Mapped[List[Book]] = \
        relationship('Book', back_populates='borrower')
