...
def add_book(session, title: str, year: int, author: str) -> None:
...
def rent_book(session: Session, email: str, book_title: str) -> None:
    """
...
def return_book(session: Session, email: str, book_title: str) -> None:
    """
    ...
    if person is None:
        raise ValueError(
            f"Somebody not registered tried to return a book: {email}")

    if book is None:
        raise ValueError(
            f"Book not found in the library {book_title}")

    # check if that person rented that book
    if not book.borrower:
        raise ValueError(
            "Nobody rents that book currently")
...
def add_person(session: Session, name: str, email: str) -> None:
    ...
    if person is not None:
        raise ValueError(
            f"person with the email: {email} already exists in the database")
...
def load_data(session: Session, file_name: str) -> None:
    # full file name
