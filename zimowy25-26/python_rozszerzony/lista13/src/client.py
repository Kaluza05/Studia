...
def parse_args():
    ...
    global_parser.add_argument("--login", type=str)
    global_parser.add_argument("--password", type=str)


    parser = argparse.ArgumentParser(
        description="Program simulating a bookstore",
    ...
    # ----- Książki -----
    book_parser = subparsers.add_parser("books", help="Book manager", parents=[global_parser])
    ...
    # ----- Osoby -----
    person_parser = subparsers.add_parser("people", help="People manager", parents=[global_parser])
    ...
    subparsers.add_parser("init-db",help="Initialize database from JSON", parents=[global_parser])

    return parser.parse_args()
   

def handle_local(session,args, logged_in = False):
    ...
    try:
        if args.model == "books":
            ...
            if args.add:
                ...
                session.commit()
            elif args.return_book and logged_in:
...
def handle_api(args, logged_in = False):
...
def main(debug = False):
    ...
    with open ("users.json") as f:
            users = json.load(f)
    logged_in = args.login and args.password and args.password == users[args.login]

    if args.mode == 'local':
        with db.SessionLocal() as session:
            handle_local(session, args, logged_in)
    elif args.mode == 'api':
            handle_api(args, logged_in)
