...
let rec choose_game_action p (g : game) (cont : command -> ans) =  
    output_string p.sock "What do you wanna do: " (fun () ->
    input_line p.sock (fun line_opt ->
      match line_opt with
      | None -> cont Resign
      | Some s ->
        ...
        | Parsed All_moves ->
          output_string p.sock ("All possible moves\n" ^ highlight_all_moves_string g) 
        ...
        | _ -> 
          output_string p.sock "Wrong action performed\n"
          (fun () -> choose_game_action p g cont)
              ))
