...
let load_texture renderer path =
  ...
  | Error (`Msg e) -> Sdl.log "Load texture error: %s" e; None
  | Ok tex -> Some tex

  

(* Rysowanie tekstury w pozycji (x, y) *)
...
let draw_board renderer tile_size =
  ...
  for x = 0 to 7 do
    for y = 0 to 7 do
      if (x + y) mod 2 = 0 then
        Sdl.set_render_draw_color renderer 255 255 255 255 |> ignore (* biały *)
      else
...
(* zaladować custom font *)
let draw_text renderer font (x, y) text =
  Printf.printf "TEXT AT: x=%d y=%d\n%!" x y;
...
let draw_lobby renderer font join_button =
  ...
  draw_text renderer font (x1, y1) "Join queue";
  Sdl.render_present renderer

let draw_queue renderer font leave_button =
  Sdl.set_render_draw_color renderer 255 255 255 255 |> ignore;
  Sdl.render_clear renderer |> ignore;
  draw_rect renderer leave_button;
  let x1, y1, x2, y2 = leave_button in
  draw_text renderer font (x1, y1 ) "Leave queue";
  Sdl.render_present renderer
...
| Ok () ->
  ...
  | Ok () ->
    ...
    match Sdl.create_window ~w:700 ~h:700 "SDL OpenGL" Sdl.Window.opengl with
    ...
    | Ok window ->
        match Sdl.create_renderer window ~index:(-1) ~flags:Sdl.Renderer.accelerated with
          ...
          | Ok renderer ->
            let image = load_texture renderer "src/pieces/knight.png" in
            let leave_button = (300,300,400,400) in

            (* Rysujemy planszę *)
            draw_lobby renderer font leave_button;
            (* draw_board renderer 80;
            (* Rysujemy obrazek na polu 0,0 jeśli się załadował *)
            (match image with
            | Some tex -> 
              print_endline "zaladowalo";
              draw_texture renderer tex (240, 240) ~resize:(80,80)
            | None -> 
              print_endline "nie zaladowalo"); *)
            
            Sdl.render_present renderer;
            Sdl.delay 5000l;
            (match image with Some tex -> Sdl.destroy_texture tex | None -> ());
...
let () =
  ...
  Tsdl_image.init Tsdl_image.Init.png |> ignore;
  ...
  let texture =
    load_texture renderer "pieces/knight.png"
    |> Result.get_ok
  in

  let dst = Sdl.Rect.create ~x:100 ~y:100 ~w:60 ~h:60 in
  ...
  let rec loop () =
    ...
    Sdl.set_render_draw_color renderer 255 255 255 255 |> ignore;
    Sdl.render_clear renderer |> ignore;
    Sdl.render_copy renderer texture ~src:dst ~dst:dst |> ignore;
    Sdl.render_present renderer;
