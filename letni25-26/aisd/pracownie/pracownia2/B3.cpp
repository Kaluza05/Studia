...
pair<string,int> find_towers(int n, vector<int>& hs) {
    ...
    vector<vector<PossConfig>> dp(2,vector<PossConfig>(h_sum+1));
    ...
    for(int i = 1; i < n; i++) {
        dp[parity_now][hs[i]] = {true,hs[i],0};

        for(int j = 0; j <= prefix_h_sum[i]; j++) {
            PossConfig t1 = dp[parity_prev][j];
            ...
            PossConfig t3 = dp[parity_prev][abs(j-hs[i])];
            ...
            dp[parity_now][j] = choose_highest(t1,t2,t3);
    ...
    vector<PossConfig> last_row = dp[parity_prev];
