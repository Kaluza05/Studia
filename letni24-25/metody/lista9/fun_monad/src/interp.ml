...
module HeapComp : COMP = struct
  type 'a comp = heap -> 'a * heap
  let return (v : 'a) : 'a comp = fun h -> (v, h)
  let bind (c : 'a comp) (f : 'a -> 'b comp) : 'b comp = fun h -> let (v, h) = c h in f v h
  ...
  let assgn (l : Loc.t) (v : value) : unit comp =
    fun h -> ((), H.add l v h)
  let final_return c = fst( c H.empty)
end
