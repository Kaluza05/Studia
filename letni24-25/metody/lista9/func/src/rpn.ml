...
let binop_of_string (s : string) : Ast.bop = 
  match s with
  | "+" -> Add
  | "-" -> Sub
  | "*" -> Mult
...
let rec process_text (text : string list) : prog=
match text with
| [] -> []
| i::p' -> (match int_of_string_opt i with
| Some i -> PushInt i :: process_text p'
| None -> Binop (binop_of_string i):: process_text p')

let of_rpn (text : string) : Ast.expr =
  let rec it (p : prog) (s : Ast.expr list) : Ast.expr = 
    match p,s with 
    | [] , [s] -> s
    | [] , _::_ ->  failwith "niepusty stos"
    | PushInt i :: p' , _ -> it p' (Int i :: s)
    | Binop op :: p' , e1 :: e2 :: s' -> it p' (Binop(op , e2 , e1) :: s')
    | _ ,_ -> failwith "blad"
  in it (process_text (split text)) []
