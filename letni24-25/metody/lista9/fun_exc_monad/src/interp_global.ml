...
module type COMP = sig
  ...
  val (let*) : 'a comp -> ('a -> 'b comp) -> 'b comp 
  (* ? *)
...
module GlobalComp : COMP = struct
  type 'a comp = heap -> ('a option) * heap
  let return (v : 'a) : 'a comp = fun h -> (Some v, h)
  let fail = fun h -> (None,h)
  let bind (c : 'a comp) (f : 'a -> 'b comp) : 'b comp = fun h -> let (v, h) = c h in 
  (match v with
  | Some v -> f v h
  | None -> (None,h))
  ...
  let final_return (c : 'a comp) : 'a =
    ...
    | Some v,_ -> v
    | None,_ -> failwith "blad w final_return"
end
...
(*interp "let x = ref 1 in try let z = x:=!x + 1 in throw with !x";;  *)