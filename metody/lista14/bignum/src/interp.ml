...
(* interpreter *)
let div_bignum v1 v2 h =
  match v1,v2 with
  | VInt i1, VInt i2 -> (match Bignum.to_bigint_opt i1, Bignum.to_bigint_opt i2 with
    | Some i1, Some i2 -> (VInt (Bignum.of_bigint (Bigint.(/) i1 i2)),h)
    | _ -> failwith "wrong")
...
let eval_op (op : bop) : value -> value -> value comp =
  ...
  | Eq   -> (fun v1 v2 h -> 
      match v1,v2 with
      | VBool t1, VBool t2 -> (VBool ((Stdlib.(=) t1 t2)),h)
      | VInt i1, VInt i2 -> (VBool (i1 = i2), h)
      | _ -> failwith "wrong types")
  | Neq  -> (fun v1 v2 h -> match v1,v2 with
      | VBool t1, VBool t2 -> (VBool ((Stdlib.(<>) t1 t2)),h)
      | VInt i1, VInt i2 -> (VBool (i1 <> i2), h)
      | _ -> failwith "wrong types")
  | Leq  -> cmp_op ( <= )
