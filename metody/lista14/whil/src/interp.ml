...
let dpre (x : ident) (op : int -> int -> int) : value comp = 
  let* v  = deref x in match v with
  | VInt n ->
    let new_val = VInt(op n 1) in
    let* _ = assgn x new_val in
    return v
  | _ -> failwith "wrong" 


let dpost (x : ident) (op :  int -> int -> int) : value comp = 
  let* v  = deref x in match v with
  | VInt n ->
    let new_val = VInt(op n 1) in
    let* _ = assgn x new_val in
...
let rec fold_m (f : 'a -> unit comp) (xs : 'a list) : unit comp =
  match xs with
  | [] -> return ()
  | x :: xs' ->
      let* _ = f x in
...
let rec eval_stmt (s : stmt) : unit comp =
  ...
  | Local(_x, _e) -> failwith "not implemented"
