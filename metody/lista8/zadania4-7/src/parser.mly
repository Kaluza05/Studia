...
mexpr:
    ...
    | FUN; ids = ident_list ; ARR ; e = mexpr { make_fun ids e}
    | FUNREC; f = IDENT; x = IDENT; ARR; e = mexpr { Funrec(f, x, e) }
    | LETREC; f = IDENT; x = IDENT; EQ; e1 = mexpr ;IN; e2 = mexpr{ Let(f,Funrec(f,x,e1),e2)}
    | FUNFIX; f = IDENT; x = IDENT; ARR; e = mexpr { 
            Let("#fix",
                Fun ("f", App (Fun ("x", Fun ("y", App (App (Var "f",
                App (Var "x", Var "x")),
                Var "y"))), Fun ("x", Fun ("y",App(App (Var "f",
                App (Var "x", Var "x")), Var "y"))))) ,
            Let("#f",
                Fun(f, Fun(x, e)), 
            Fun(x, App(App(Var "#fix", Var "#f"),Var x))))}
    | e = expr { e }
    ;
    
ident_list:
    | x = IDENT { [x] }
    | x = IDENT ; e = ident_list { x::e }
    ;
expr:
