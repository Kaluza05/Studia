...
Fixpoint it (xs : list nat) (ys : list nat) : list nat :=
    match xs with 
    | [] => ys
    | x::xs => it xs (insert ys x)
    end.
Definition insert_sort (xs : list nat) := it xs [].
...
Lemma it_sorted (xs : list nat) :
    forall ys ,sorted_non_desc ys -> sorted_non_desc (it xs ys).
Proof.
    induction xs; simpl.
    +tauto.
    +intros ys Y. apply IHxs. apply insert_spec2. exact Y.
Qed.

Lemma insert_sort_in_sorted (xs : list nat)  :
     sorted_non_desc (insert_sort xs).
Proof.
    unfold insert_sort. apply it_sorted. constructor.
Qed.
...
Lemma perm_insert (ys : list nat) (zs : list nat) (x : nat) :
    perm ys zs -> perm (insert ys x) (insert zs x).
Proof.
    intro H.
    apply perm_trans with (x :: ys).
    +apply perm_symm. apply perm_spec1.
    +apply perm_trans with (x :: zs).
    -apply perm_extend. assumption.
    -apply perm_spec1.
Qed.

Lemma perm_it_1 (xs : list nat): 
    forall ys zs, perm ys zs -> perm (it xs ys) (it xs zs).
Proof.
    induction xs.
    +simpl. tauto.
    +simpl. intros ys zs H.
    apply IHxs.
    apply perm_trans with (a :: ys).
    -apply perm_symm. apply perm_spec1.
    -apply perm_trans with (a :: zs).
    *apply perm_extend. assumption.
    *apply perm_spec1.
